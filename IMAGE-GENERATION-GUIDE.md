...
4. **Émergence visuelle** : L'image cohérente émerge naturellement des interactions locales
...
| Mémoire | Très faible | Énorme (10GB+) |
| Vitesse | Très rapide (instantané) | Lente (30-60s) |
| Parallélisation | Triviale | Complexe |
| Contrôle créatif | Très fin (par région) | Global/basique |
...
type PixelAtom struct {
    ...
    State              float64            // État interne [0, 1]
    ...
    IsFrozen           bool               // État de veille énergétique
}
```
...
Chaque atome reçoit ses modifications basées sur position + terme
```
...
- **CouplingCoefficient (α)** : Force d'interaction voisinage (0.7)
- **DecayFactor (δ)** : Affaiblissement des faibles connexions (0.05)

### Apprentissage

- **ReinforcementFactor (γ)** : Renforcement des poids (0.15)
- **ResonanceSensitivity (σ)** : Sensibilité à l'alignement (0.1)
...
Les contraintes peuvent être appliquées spatialement (haut, bas, centre, etc.).
...
# Le système apprend les préférences et réaffine
```
...
- **`database/image_atomic.go`** : Cœur du moteur
- **`image_commands.go`** : Interface CLI
- **`main.go`** : Intégration dans le système
...
(Millions de paramètres, apprentissage centralisé)
```
...
(Pas de paramètres apprenables, émergence locale)
```
...
L'extension de T.R.A. aux images crée un système de génération visuelle :
...
- ✅ **Contrôlable** : Prompt très fin, région-spécifique
- ✅ **Beau** : Émerge naturellement d'interactions locales

C'est la fusion de la physique (résonance), de la biologie (autonomie locale), et de l'informatique (asynchrone parallèle).
