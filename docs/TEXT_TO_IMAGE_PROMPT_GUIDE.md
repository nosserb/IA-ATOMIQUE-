...
Tokenization  [for√t, myst√©rieuse, arbres, luminescents]
...
- Atoms propagate states locally (resonance Œ)
- Follow neural pattern (pattern Œ≤)
- Smooth colors locally (smoothing Œ≥)
...
| **prompt** | Any French/English text | "for√t myst√©rieuse" |

## Examples

### Fast Generation (Low Detail)
```bash
./programme generate from-prompt 256 256 100 "une montagne enneig√©e"
# Time: ~2-3 seconds
...
```bash
./programme generate from-prompt 512 512 300 "oc√©an temp√©tueux avec des vagues"
# Time: ~12-15 seconds
...
### Complex Scene
```bash
./programme generate from-prompt 512 512 200 "ch√teau m√©di√©val dans la brume avec des torches"
...
| **BUSINESS** | 3 | commerce, √©conomie, march√©, entreprise, vente, affaires |
| **ALIMENTATION** | 4 | nourriture, fruit, cuisine, restaurant, manger, pain, g√teau |
| **SANT√** | 5 | m√©decine, sant√©, h√¥pital, docteur, maladie, cure |
...
### Be Descriptive
 **Good**: "oc√©an temp√©tueux avec vagues g√©antes et temp√te"
...
### Mixing Categories
 "technologie futuriste dans un ch√teau m√©di√©val"
   - Mixes TECH + HISTOIRE
...
### Emotion & Adjectives
 "for√t sombre, myst√©rieuse, ancienne"
...
./programme generate from-prompt 512 512 150 "for√t myst√©rieuse"
...
./programme generate from-prompt 256 256 100 "for√t sombre myst√©rieuse"
...
- Œ (resonance) = 0.3 (moderate cooperation)
- Œ≤ (pattern) = 0.5 (balance pattern/emergence)
- Œ≥ (smoothing) = 0.2 (moderate smoothing)
- Œµ (damping) = 0.9 (stable convergence)
...
### Nature Prompts
```bash
./programme generate from-prompt 512 512 200 "montagne enneig√©e sous la lune"
...
### Tech Prompts
```bash
./programme generate from-prompt 512 512 200 "r√©seau de neurones et algorithmes"
...
### Food Prompts
```bash
./programme generate from-prompt 256 256 100 "festin m√©di√©val avec pain et fruits"
