...
```
Input: "une for�t mysterieuse avec des arbres luminescents"
         
Tokenization  [for�t, mysterieuse, arbres, luminescents]
...
- Follow neural pattern (pattern beta)
- Smooth colors locally (smoothing gamma)
...
| **prompt** | Any French/English text | "for�t mysterieuse" |

## Examples

### Fast Generation (Low Detail)
```bash
./programme generate from-prompt 256 256 100 "une montagne enneigee"
# Time: ~2-3 seconds
...
```bash
./programme generate from-prompt 512 512 300 "ocean tempetueux avec des vagues"
# Time: ~12-15 seconds
...
### Complex Scene
```bash
./programme generate from-prompt 512 512 200 "ch�teau medieval dans la brume avec des torches"
...
| **BUSINESS** | 3 | commerce, economie, marche, entreprise, vente, affaires |
| **ALIMENTATION** | 4 | nourriture, fruit, cuisine, restaurant, manger, pain, g�teau |
| **SANT�** | 5 | medecine, sante, hopital, docteur, maladie, cure |
...
### Be Descriptive
 **Good**: "ocean tempetueux avec vagues geantes et temp�te"
...
### Mixing Categories
 "technologie futuriste dans un ch�teau medieval"
   - Mixes TECH + HISTOIRE
...
### Emotion & Adjectives
 "for�t sombre, mysterieuse, ancienne"
...
./programme generate from-prompt 512 512 150 "for�t mysterieuse"
...
./programme generate from-prompt 256 256 100 "for�t sombre mysterieuse"
...
- beta (pattern) = 0.5 (balance pattern/emergence)
- gamma (smoothing) = 0.2 (moderate smoothing)
- epsilon (damping) = 0.9 (stable convergence)
...
### Nature Prompts
```bash
./programme generate from-prompt 512 512 200 "montagne enneigee sous la lune"
...
### Tech Prompts
```bash
./programme generate from-prompt 512 512 200 "reseau de neurones et algorithmes"
...
### Food Prompts
```bash
./programme generate from-prompt 256 256 100 "festin medieval avec pain et fruits"
