...
P_ij(t+1) = P_ij(t)    [Current value]
          + ��Σ W��P   [Weighted neighbor influence]
          + beta�V(t)     [Momentum from previous step]

Where:
  �P = P_kl - P_ij    (color difference to neighbor)
  W = connection weight (learned)
  � = diffusion strength (0.15)
  beta = momentum damping (0.95)
...
- Different colors (large �P): drive bigger change
- Diffusion coefficient �: controls how fast
...
```
gamma = 0.05          (Reinforcement rate)
||�P||^2 = (�R)^2 + (�G)^2 + (�B)^2    (Color distance)

When ||�P||^2 is small:
  exp(-||�P||^2)  1        (maximum reinforcement)
  W increases by +gamma

When ||�P||^2 is large:
  exp(-||�P||^2)  0        (no reinforcement)
  W stays same
```
...
```
Iteration 1:
  Similar pixels (�P small)      W increases
  Different pixels (�P large)    W stays
...
```
PATTERN_EMERGENCE(reference_image, width, height, iterations, seed_density, �, gamma):
  ...
  2. FOR t = 0 TO iterations:
     ...
     a) DIFFUSION STEP
        FOR each pixel (i,j):
          ...
          P[i,j]  P[i,j] + � * influence + beta * V[i,j]
          V[i,j]  � * influence  (momentum)
     ...
     c) LOSS COMPUTATION
        L  0
        FOR each seed:
          error  ||P[sy, sx] - color||^2
          L += error / num_seeds
     
     d) REINFORCEMENT (every 5 steps)
        FOR each connection (i,j)  (k,l):
          delta  ||P[i,j] - P[k,l]||^2
          W[i,j;k,l]  W[i,j;k,l] + gamma * exp(-delta)
...
- $\alpha$ (larger �  faster convergence)
- $\gamma$ (larger gamma  faster stabilization)
...
�=0.10, gamma=0.05: 0.89      0.34       0.32       (plateau)
�=0.15, gamma=0.05: 0.78      0.28       0.27       (plateau)
�=0.20, gamma=0.05: 0.65      0.25       0.24       (plateau)
�=0.15, gamma=0.01: 0.88      0.58       0.52       (slow)
�=0.15, gamma=0.10: 0.72      0.22       0.20       (fast)
```
...
Causes:
  1. � too large (diffusion too strong)
...
Decision:
  ...
  - Quality poor?  Adjust parameters (�, gamma, seeds)
```
...
```
Scale 1: 256�256, �=0.20, gamma=0.05, 100 iter
         (fast coarse pattern)
         
Scale 2: 512�512, �=0.15, gamma=0.05, 200 iter
         (refine with higher resolution)
         
Scale 3: 1024�1024, �=0.10, gamma=0.05, 300 iter
...
Early phase (iterations 0-50):
  � = 0.25  (spread fast)
  gamma = 0.02  (weak learning)

Middle phase (iterations 50-150):
  � = 0.15  (balanced)
  gamma = 0.05  (normal learning)
...
3. **Learning**: $W(t+1) = W(t) + \gamma \exp(-||�P||^2)$
