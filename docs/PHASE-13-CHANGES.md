...
**After**:
```go
// === PHASE 13+++: Fenπtrage strict - Force lexical diversity between consecutive blocks ===
result := []BlocVectoriel{}
for i, bloc := range r.Blocs {
    if selectedIndices[i] {
...
func CalculerSimilarityVocabLexical(mots1, mots2 []string) float64 {
    ...
    // Calculate Jaccard similarity: |A π B| / |A  B|
    intersection := 0
...
var SynonymsDict = map[string][]string{
    ...
    "πtre":         {"constituer", "représenter", "figurer", "πtre"},
    ...
    "monde":        {"univers", "domaine", "sphπre", "monde"},
...
5.  Added strict fenπtrage logic
