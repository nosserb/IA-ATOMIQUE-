...
└─ Dictionnaire de 30+ paires mot→[synonymes]
   ...
   ├─ Adverbes: très, beaucoup, peu, souvent
   └─ Noms: chose, façon, temps, problème
...
- Mots longs: % de mots > 8 caractères
...
2. Pour chaque mot du dictionnaire:
   ...
   - Remplacer première occurrence avec regex
   - Préserver casse (première lettre majuscule)
...
**Exemple d'exécution:**
```
Input:  "Les ordinateurs font très vite"
Hash("très") % 3 = 0 → "extrêmement"
Hash("vite") % X = Y → "rapidement"
Output: "Les ordinateurs accomplissent extrêmement rapidement"
```
...
5. Afficher "[HUMANISATION AVANCÉE COMPLÉTÉE]"
```
...
```
Texte original: "Les ordinateurs modernes sont très puissants"

Standard:       "Les ordinateurs modernes sont très puissants. Ensuite..."
                (ajoute connecteur)

Professionnel:  "Les ordinateurs modernes sont particulièrement puissants"
                (remplace "très" → "particulièrement")

Avancé:         "Les ordinateurs modernes sont extrêmement puissants"
                (remplace "très" → "extrêmement", autre synonyme)
```
...
1. **Modulaire**: Chaque fonction peut être utilisée indépendamment
...
6. **Compatible**: N'interfère pas avec les modes standard/professionnel
