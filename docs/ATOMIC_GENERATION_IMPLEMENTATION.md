...
- **GenerationGrid**: Main container (Width � Height grid)
  - Parameters: � (resonance), beta (pattern), gamma (smoothing), delta (feedback), epsilon (damping)
  - Thread-safe with RWMutex
  - Target image support for feedback mode
  
- **Key Functions**:
  - `PropagateLocal()`: s_ij(t+1) = ��Σ neighbors + beta�P_ij + epsilon�V
  - `StateToColor()`: HSL-based stateRGB conversion
  - `SmoothColors()`: Local color averaging C' = (1-gamma)�C + gamma�neighbor_avg
...
for s := 0; s < 5; s++ {
    ...
    newCells[i][j].State[s] = 
        ...
        gg.PatternBeta * pattern[s] +                // beta�P_ij
        newCells[i][j].Velocity[s]                   // epsilon�V
...
**Fine Detail**: �=0.2, beta=0.6, gamma=0.10  Sharp edges, local patterns
**Smooth Blend**: �=0.4, beta=0.4, gamma=0.25  Natural-looking, coherent
**Wave-Centric**: �=0.1, beta=0.8, gamma=0.05  Pattern dominates
**Emergent**: �=0.5, beta=0.3, gamma=0.20  Creative, self-organizing
