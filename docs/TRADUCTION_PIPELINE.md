...
Le système implémente un pipeline avancé de traduction et extraction de phrases clés combining:
...
### Énergie avec facteur de confiance
```
E(Pi) = Σ αk·f(wk)
    où αk = poids syntaxique (1.5 pour sujet, 1.3 pour verbe, 1.0 défaut)
...
type Phrase struct {
    ...
    Energie         float64     // E(Pi) intrinsèque
...
### Entrée
```
Texte = "The file contains important data. Le document est très important."
```

### Étape 1: Découpage
```
P1: "The file contains important data"
P2: "Le document est très important"
```
...
### Étape 3-4: Énergie
```
E(P1) = 0.7  (mots : file/fichier→1.0, contains→0.6, data/données→1.0)
Etotal(P1) = 0.7 × 0.8 + 0.2 × sim(P1,P2) = 0.56 + coeff·sim

E(P2) = 0.9  (mots : document→1.0, important→1.0)
Etotal(P2) = 0.9 × 1.0 + 0.2 × sim(P2,P1) = 0.90 + coeff·sim
```

### Étape 5: Filtrage
```
ϵ = μ(Etotal) - σ(Etotal) ≈ 0.73 - 0.08 = 0.65
...
Rfinal = ["Le document est très important"]
```

## Avantages du système

✅ **Normalisation globale**: Tous les textes traités en français
✅ **Qualité préservée**: Texte original français non dégradé (γi = 1.0)
...
3. **Pas de context**: Traductions word-by-word, pas de phrase complète
4. **Pas de homophones**: "bat" (animal) vs "bat" (sport) traités pareil

### Améliorations recommandées
1. **Intégration LibreTranslate/DeepL** pour traductions complètes
...
Le pipeline s'intègre parfaitement:
